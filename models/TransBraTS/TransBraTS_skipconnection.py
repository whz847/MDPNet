...
class Grade_netwoek(nn.Module):
    def __init__(self):
        ...
        self.idh_weight = nn.Parameter(torch.Tensor([1.0]))
        self.atrx_weight = nn.Parameter(torch.Tensor([1.0]))
        self.p19q_weight = nn.Parameter(torch.Tensor([1.0]))

        self.avg_pool_3d = nn.AvgPool3d(16, 1)
        self.max_pool_3d = nn.MaxPool3d(16, 1)
        self.Hidder_layer_1 = nn.Linear(1280, 512)
        self.Hidder_layer_2 = nn.Linear(512, 256)
        self.Hidder_layer_3 = nn.Linear(256, 32)
        self.drop_layer = nn.Dropout(p=0.2)
        self.classifier_idh = nn.Linear(32, 2)
        self.classifier_p19q = nn.Linear(32, 2)
        self.classifier_atrx = nn.Linear(32, 2)
        self.classifier_type = nn.Linear(35, 4)
    ...
    def forward(self, x4_1, encoder_output):
        ...
        x = self.Hidder_layer_3(x)
        # 分别计算IDH、ATRX和1p19q的预测结果
        idh_pred=torch.softmax(self.classifier_idh(x),dim=1)
        atrx_pred=torch.softmax(self.classifier_atrx(x),dim=1)
        p19q_pred=torch.softmax(self.classifier_p19q(x),dim=1)
        # 根据可学习的权值进行加权处理
        idh_pred_weighted = idh_pred * self.idh_weight
        atrx_pred_weighted = atrx_pred * self.atrx_weight
        p19q_pred_weighted = p19q_pred * self.p19q_weight
        # 将加权结果拼接起来作为Type分类的输入
        type_input = torch.cat((x,idh_pred_weighted, atrx_pred_weighted, p19q_pred_weighted), dim=1)
        # 进行Type分类
        type_pred = torch.softmax(self.classifier_type(type_input), dim=1)

        #y = self.classifier(x)
        return idh_pred,atrx_pred,p19q_pred,type_pred
...
class FeatureFusionModule(torch.nn.Module):
    ...
    def forward(self, input_1, input_2):
        ...
        return x
#IDH和ATRX的结果对1p/19q的结果有影响的情况
class IDH_ATRX_p19q_type_network(nn.Module):
    def __init__(self):
        super(IDH_ATRX_p19q_type_network, self).__init__()

        self.avg_pool_3d = nn.AvgPool3d(16, 1)
        self.max_pool_3d = nn.MaxPool3d(16, 1)
        # self.Hidder_layer_0 = nn.Linear(3328,2280)
        self.Hidder_layer_1 = nn.Linear(1792,1280)
        self.Hidder_layer_idh = nn.Linear(1280, 512)
        self.Hidder_layer_atrx = nn.Linear(1280, 512)
        self.Hidder_layer_p19q = nn.Linear(1282,512)
        self.Hidder_layer_idh2 = nn.Linear(512, 32)
        self.Hidder_layer_atrx2 = nn.Linear(512, 32)
        self.Hidder_layer_p19q2 = nn.Linear(512, 32)
        self.drop_layer = nn.Dropout(p=0.2)
        self.classifier_idh = nn.Linear(32, 2)
        self.classifier_p19q = nn.Linear(32, 2)
        self.classifier_atrx = nn.Linear(32, 2)
    ...
    def forward(self,x4_1,encoder_output,x_second):
        x = self.feature_fusion_layer(x4_1,encoder_output,x_second)
        x = self.drop_layer(x)
        # x = self.Hidder_layer_0(x)
        x = self.Hidder_layer_1(x)
        x_p19q=x
        x_idh = self.Hidder_layer_idh(x)
        x_atrx=self.Hidder_layer_atrx(x)
        x_idh2 = self.Hidder_layer_idh2(x_idh)
        x_atrx2 = self.Hidder_layer_atrx2(x_atrx)
        # 分别计算IDH和ATRX的预测结果
        idh_pred = torch.softmax(self.classifier_idh(x_idh2), dim=1)
        atrx_pred = torch.softmax(self.classifier_atrx(x_atrx2), dim=1)
        # 寻找IDH和ATRX的预测结果
        y, _ = torch.max(idh_pred, dim=1)
        z, _ = torch.max(atrx_pred,dim=1)
        #将IDH和ATRX的预测结果和原来的1p/19q的1280个神经单元连接起来
        p19q_cat = torch.cat([x_p19q, y.unsqueeze(1),z.unsqueeze(1)], dim=1)
        x_p19q2=self.Hidder_layer_p19q(p19q_cat)
        x_p19q3=self.Hidder_layer_p19q2(x_p19q2)
        p19q_pred = torch.softmax(self.classifier_p19q(x_p19q3), dim=1)
        return idh_pred,atrx_pred,p19q_pred

#IDH和ATRX的结果对1p/19q的结果无影响的情况
# class IDH_ATRX_p19q_type_network(nn.Module):
#     def __init__(self):
#         super(IDH_ATRX_p19q_type_network, self).__init__()
#
...
#         self.Hidder_layer_idh = nn.Linear(1280, 512)
#         self.Hidder_layer_atrx = nn.Linear(1280, 512)
#         self.Hidder_layer_p19q = nn.Linear(1280, 512)
#         self.Hidder_layer_idh2 = nn.Linear(512, 32)
#         self.Hidder_layer_atrx2 = nn.Linear(512, 32)
#         self.Hidder_layer_p19q2 = nn.Linear(512, 32)
#         self.drop_layer = nn.Dropout(p=0.2)
...
#         x = self.Hidder_layer_1(x)
#         x_idh = self.Hidder_layer_idh(x)
#         x_p19q = self.Hidder_layer_p19q(x)
#         x_atrx = self.Hidder_layer_atrx(x)
#         x_idh2 = self.Hidder_layer_idh2(x_idh)
#         x_p19q2 = self.Hidder_layer_p19q2(x_p19q)
#         x_atrx2 = self.Hidder_layer_atrx2(x_atrx)
#         # 分别计算预测结果
#         idh_pred = torch.softmax(self.classifier_idh(x_idh2), dim=1)
#         atrx_pred = torch.softmax(self.classifier_atrx(x_atrx2), dim=1)
#         p19q_pred = torch.softmax(self.classifier_p19q(x_p19q2), dim=1)
#         return idh_pred, atrx_pred, p19q_pred

#ATRX的结果对1p/19q的结果有影响的情况
# class IDH_ATRX_p19q_type_network(nn.Module):
#     def __init__(self):
#         super(IDH_ATRX_p19q_type_network, self).__init__()
#
#         self.avg_pool_3d = nn.AvgPool3d(16, 1)
#         self.max_pool_3d = nn.MaxPool3d(16, 1)
#         # self.Hidder_layer_0 = nn.Linear(2816,1792)
#         self.Hidder_layer_1 = nn.Linear(1792,1280)
#         self.Hidder_layer_idh = nn.Linear(1280, 512)
#         self.Hidder_layer_atrx = nn.Linear(1280, 512)
#         self.Hidder_layer_p19q = nn.Linear(1281,512)
#         self.Hidder_layer_idh2 = nn.Linear(512, 32)
#         self.Hidder_layer_atrx2 = nn.Linear(512, 32)
#         self.Hidder_layer_p19q2 = nn.Linear(512, 32)
#         self.drop_layer = nn.Dropout(p=0.2)
...
#         # 寻找IDH和ATRX的预测结果
#         z, _ = torch.max(atrx_pred,dim=1)
#         #将ATRX的预测结果和原来的1p/19q的1280个神经单元连接起来
#         p19q_cat = torch.cat([x_p19q,z.unsqueeze(1)], dim=1)
#         x_p19q2=self.Hidder_layer_p19q(p19q_cat)
#         x_p19q3=self.Hidder_layer_p19q2(x_p19q2)
#         p19q_pred = torch.softmax(self.classifier_p19q(x_p19q3), dim=1)
#         return idh_pred,atrx_pred,p19q_pred

# IDH的结果对1p/19q的结果有影响的情况
# class IDH_ATRX_p19q_type_network(nn.Module):
#     def __init__(self):
#         super(IDH_ATRX_p19q_type_network, self).__init__()
...
#         return idh_pred,atrx_pred,p19q_pred
    ...
    def feature_fusion_layer(self,x4_1,encoder_output,x_second):
        x4_1_avg = self.avg_pool_3d(x4_1)
        x4_1_max = self.max_pool_3d(x4_1)
        ...
        return torch.cat([x4_1_avg,x4_1_max,encoder_avg,encoder_avg_max,x_second], dim=1)
...
class TransformerBraTS(nn.Module):
    def __init__(
        ...
        self.Unet = Unet(in_channels=4, base_channels=16, num_classes=4)#这儿的num_classes没有用到，可以删去
        self.bn = nn.BatchNorm3d(128)
        self.relu = nn.LeakyReLU(inplace=True)

    def encode(self, x,layer4):
        if self.conv_patch_representation:
            # combine embedding with conv patch distribution
            x1_1, x2_1, x3_1, x4_1,y,y1 = self.Unet(x,layer4)
            x = self.bn(x4_1)
            x = self.relu(x)
            x = self.conv_x(x)
        ...
        return x1_1, x2_1, x3_1,x4_1, x8,y,y1
    ...
    def forward(self, x,layer4):
        ...
        x1_1, x2_1, x3_1,x4_1, encoder_output,y,y1 = self.encode(x,layer4)
        return x1_1, x2_1, x3_1,x4_1, encoder_output,y,y1
