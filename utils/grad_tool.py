...
class PCGrad():
    ...
    def adjust_learning_rate(self, epoch, max_epoch, init_lr, power=0.9):
        for param_group in self._optim.param_groups:
            param_group['lr'] = round(init_lr * np.power(1 - (epoch) / max_epoch, power), 8)
    def pc_param_groups(self):
    ...
    def _project_grad(self,grads,has_grads,mean_grads):
        shared = torch.stack(has_grads).prod(0).bool()
        pc_grad, num_task = copy.deepcopy(grads), len(grads)
        g_i_g_j = torch.dot(grads,mean_grads)
        return g_i_g_j
        ...
        for g_i in pc_grad:
            ...
            for g_j in grads:
                ...
                if g_i_g_j < 0:
                    g_j_norm = g_j.norm()
                    if g_j_norm.item()==0:
                        logging.info("g_j_norm is equal to zero,please check here!!!")
                        g_i -= (g_i_g_j) * g_j / (g_j.norm() ** 2+1e-5)
                    else:
...
class Weightmodel(nn.Module):
    def __init__(self,n_task):
        ...
        self.n_task = n_task
        self.weights = torch.nn.Parameter(torch.tensor([2.0,0.0],requires_grad=True))
    def get_weight(self):
        return self.weights
